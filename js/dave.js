...
function get_products(params, callbackFunc, errorFunc) {
    ...
    //
    // Search by keyword
    // get_products({"search_terms": "~search keyword"}, function(data) { console.log(data) })
    //
    // Search by multiple factors
    // get_products({"search_terms": "~search keyword"}, function(data) { console.log(data) })
    //
    // Set page size (default page size is 50)
    // get_products({"search_terms": "~search keyword", "_page_size": 20}, function(data) { console.log(data) })
    // 
    // Set page number
    // get_products({"search_terms": "~search keyword", "_page_size": 20, "_page_number": 2}, function(data) { console.log(data) })
    //
    //
    params = params || {};
    ...
    if ( params.keywords ) {
        params["search_terms"] = params.keywords;
        delete params["keywords"];
    }
    ajaxRequestWithData("/objects/product", "GET", params, callbackFunc, errorFunc);
    if ( params.search_terms && !getCookie("searched") ) {
...
function get_product(product_id, params, callbackFunc, errorFunc) { //########## Params Was Not Passed ################
...
//This function uses POST which means one new data is entered [used to make a new enquiry list]
...
//The function uses PATCH which means old data is updated with new info [used to make update to enquiry list that is alreay created]
